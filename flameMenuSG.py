...
__version__ = 'v0.1.12 dev 001 GhostVFX'
DEBUG = False
...
class flameSuperclips(flameMenuApp):
        ...
        while self.threads:
            ...
            for pb_file in pb_files:
                ...
                if pb_file_id:
                    ...
                    self.log_debug('no id for pb file?') 
                    self.log_debug(pformat(pb_file)) 
            pb_files = list(filtered_pb_files)
            self.log_debug('retro loop filtered down to %s publishes, cache size %s' % (
                len(pb_files),
            ...
            if day < self.RETRO_LOOP:
                day += 1
            else:
                self.log_debug('retro loop completed %s days in %s sec' % (
                    self.RETRO_LOOP,
                    time.time()-start
                ))
                self.loop_timeout(timeout, start)
    ...
    def utility_loop(self, timeout):
        while self.threads:
            ...
            try:
                ...
                self.log('Superclips: utility_loop: %s' % pformat(e))
    ...
    def process_publishes(self, pb_files, loop):
        ...
        for entity_id in sorted(list(entity_ids), reverse=True):
            ...
            verified_entity_pb_files = self.verify_published_files(scanned_entity_pb_files)
            ...
            for path in superclip_path:
                if os.path.isfile(path):
                    try:
                        ...
                        self.log('unable to open superclip file %s' % path)
                        continue
                else:
                    self.log('creating superclip ** %s' % str(path)[len(self.prefs['SUPERCLIPS_FOLDER'])+1:])
                    self.write_openclip(path, superclip)
                    continue

                if openclip_file.read() != superclip:
                    self.log('updating superclip ** %s' % str(path)[len(self.prefs['SUPERCLIPS_FOLDER'])+1:])
                    self.write_openclip(path, superclip)
                else:
                    continue
    ...
    def compose_superclip(self, pb_files):
        ...
        if first_video_track is None:
            self.log('Could not find the first video track in the published clip file')
    ...
    def flame_frame_spec_from_path(self, path, file_names):
        ...
        if not match:
            self.log_debug('no match found %s\n' % path)
            return None
        ...
        # is no frame number to extract.
        if not files:
            self.log_debug('no files found %s\n' % path)
            return None
        ...
        # except:
        #    self.log_debug('value error trying to parse frame range %s' % path)
        #    return None
        ...
        if ((max_frame + 1) - min_frame) != len(frames):
            ...
            for frame in frames:
                if not current_frame in frames:
                    # self.log_debug ('%s missing' % current_frame)
                    pass
                current_frame += 1
            return None
    ...
    def guess_superclip_fps(self, pb_files):
        ...
        for project in self.active_projects.values():
            if project.get('id') == project_id:
                project_name = project.get('name', '')
                project_fps = project.get('sg_fps')
    ...
    def fps_from_rational(self, fps_rational):
        ...
        try:
            if int(float(fps_rational)) in rates.keys():
                return rates.get(int(float(fps_rational)), 25)
            else:
                self.log_debug('unknown fps %s' % fps_rational)
                return False
        except Exception as e:
            self.log_debug('unknown fps %s' % fps_rational)
            return False
    ...
    def read_exr_header(self, path):
        ...
        try:
            ...
            self.log_debug('unable to open exr file\n%s:' % path)
            return None
        ...
        try:
            ...
            self.log('error reading exr header %s' % exr.name)
            self.log(e)
            exr.close()
            return None
    ...
    def write_openclip(self, flame_clip_path, xml_string):
        ...
        if not flame_clip_path.startswith(self.prefs['SUPERCLIPS_FOLDER']):
            self.log("DANGER: attemting to write outside clip root %s" % self.prefs['SUPERCLIPS_FOLDER'])
            return False
        ...
        if not os.path.exists(os.path.dirname(flame_clip_path)):
            ...
            try:
                ...
                self.log("Failed to create a folder %s: %s" % (os.path.dirname(flame_clip_path), e))
        ...
        if not os.path.isfile(flame_clip_path):
            try:
                ...
                self.log("Failed to open file for writing %s: %s" % (flame_clip_path, e))
            try:
                fh.write(xml_string)
            except Exception as e:
                self.log("Failed to write file %s: %s" % (flame_clip_path, e))            
            finally:
                if fh:
                    fh.close()
        else:
